...
# Brief: Base Lightning Module of PCPNet
import os
import time
import yaml
import numpy as np
...
from pcpnet.models.loss import Loss
from pcpnet.utils.projection import projection
from pcpnet.utils.logger import log_point_clouds, save_range_mask_and_semantic, save_point_clouds
...
class BasePredictionModel(LightningModule):
    ...
    def on_load_checkpoint(self, checkpoint):
        # load the semantic weights here
        for param_name in self.loss.state_dict():
            checkpoint['state_dict']['loss.' + param_name] = self.loss.state_dict()[param_name]

    def on_save_checkpoint(self, checkpoint):
        # pop the semantic weights here
        for param_name in self.loss.state_dict():
            del checkpoint['state_dict']['loss.' + param_name]
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log("train/loss_semantic", loss["loss_semantic"])
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss, learning_map_inv, color_map, output_argmax, target_argmax, label, \
        rand_t, similarity_output, similarity_target = self.loss(
            output, future, "val", self.current_epoch
        )
        ...
        self.log("val/loss_semantic", loss["loss_semantic"], on_epoch=True)

        selected_sequence_and_frame = self.cfg["VALIDATION"]["SELECTED_SEQUENCE_AND_FRAME"]
    ...
    def test_step(self, batch, batch_idx):
        ...
        if not self.cfg["TEST"]["ONLY_SAVE_POINT_CLOUDS"]:
            loss, learning_map_inv, color_map, output_argmax, target_argmax, label, \
            rand_t, similarity_output, similarity_target = self.loss(
                output, future, "test", self.current_epoch
            )

            self.log("test/loss_range_view", loss["loss_range_view"], on_epoch=True)
            self.log("test/loss_mask", loss["loss_mask"], on_epoch=True)
            self.log("test/loss_semantic", loss["loss_semantic"], on_epoch=True)
            self.log("test/similarity_output", similarity_output.detach(), on_epoch=True)
            self.log("test/similarity_target", similarity_target.detach(), on_epoch=True)
            ...
            selected_sequence_and_frame = self.cfg["TEST"]["SELECTED_SEQUENCE_AND_FRAME"]
            sequence_batch, frame_batch = batch["meta"]
            for sample_idx in range(frame_batch.shape[0]):
                sequence = sequence_batch[sample_idx].item()
                frame = frame_batch[sample_idx].item()
        ...
        if self.cfg["TEST"]["SAVE_POINT_CLOUDS"] or self.cfg["TEST"]["ONLY_SAVE_POINT_CLOUDS"]:
    ...
    def test_epoch_end(self, outputs):
        ...
        testdir = os.path.join(self.cfg["LOG_DIR"], self.cfg["TEST"]["DIR_NAME"])
