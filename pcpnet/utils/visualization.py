...
# Brief: Visualization of Point Cloud Predictions with Open3D
...
def get_car_model(filename):
    ...
    mesh.scale(0.001, (-0.5, 0.4, -1.2))
    R = mesh.get_rotation_matrix_from_quaternion(np.array([0, 0, 1, 1]).T)
    mesh.rotate(R)
...
class Visualization:
    ...
    def show_or_hide_pred(self, vis):
        if time.time() - self.last_time_key_pressed > SLEEPTIME:
            self.last_time_key_pressed = time.time()
            if self.pred_flag:
    ...
    def update(self, vis):
        ...
        if self.gt_flag:
            geometry_list.append(gt_pcd)
        if self.pred_flag:
            geometry_list.append(pred_pcd)
        if self.car_mesh:
            if self.car_flag:
    ...
    def _register_key_callbacks(self):
        ...
        self.register_key_callback("G", self.show_or_hide_gt)
        self.register_key_callback("F", self.show_or_hide_pred)
        self.register_key_callback("D", self.show_or_hide_car)
    ...
    def print_help(self):
        ...
        print("J: previous prediction step")
        print("G: show or hide gt point cloud")
        print("F: show or hide pred point cloud")
        print("D: show or hide car model")
        print("S: start")
        print("X: stop")
