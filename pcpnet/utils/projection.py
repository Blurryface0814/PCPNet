...
class projection:
    ...
    def get_xyz_from_range_view(self, range_view):
        """Reproject from range image to xyz

        Args:
            range_view (torch.tensor): Range image with size (B, T, H, W)

        Returns:
            torch.tensor: Valid 3D points with size (B, T, 3, H, W)
        """
        B, T, H, W = range_view.shape
        points = torch.zeros(B, T, H, W, 3).type_as(range_view)
        points[:, :, :, :, 0] = range_view * self.x_fac.type_as(range_view)
        points[:, :, :, :, 1] = range_view * self.y_fac.type_as(range_view)
        points[:, :, :, :, 2] = range_view * self.z_fac.type_as(range_view)
        return points.permute(0, 1, 4, 2, 3)
