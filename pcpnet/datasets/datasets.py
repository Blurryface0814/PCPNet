...
# Brief: Dataset Implementation for KITTI Odometry
import os
import yaml
import torch
import numpy as np
...
from pcpnet.utils.preprocess_data import prepare_data, compute_mean_and_std
from pcpnet.utils.utils import load_files


class KittiOdometryModule(LightningDataModule):
    """A Pytorch Lightning module for KITTI Odometry"""

    def __init__(self, cfg, dataset_path, rawdata_path=None):
    ...
    def prepare_data(self):
        ...
        if self.cfg["DATA_CONFIG"]["GENERATE_FILES"]:
            prepare_data(self.cfg, self.dataset_path, self.rawdata_path)

    def setup(self, stage=None):
        """Dataloader and iterators for training, validation and test data"""
        ########## Point dataset splits
        train_set = KittiOdometryRaw(self.cfg, split="train", dataset_path=self.dataset_path)

        val_set = KittiOdometryRaw(self.cfg, split="val", dataset_path=self.dataset_path)

        test_set = KittiOdometryRaw(self.cfg, split="test", dataset_path=self.dataset_path)
...
class KittiOdometryRaw(Dataset):
    ...
    def __init__(self, cfg, split, dataset_path=None):
        ...
        for seq in self.sequences:
            ...
            scan_path_intensity = os.path.join(self.root_dir, seqstr, "processed", "intensity")
            ...
            self.filenames_label[seq] = load_files(scan_path_label)
            assert len(self.filenames_range[seq]) == len(self.filenames_label[seq])
    ...
    def __getitem__(self, idx):
        ...
        past_filenames_label = self.filenames_label[seq][from_idx: to_idx + 1]
        ...
        for t in range(self.n_past_steps):
            ...
            past_data[5, t, :, :] = self.load_label(past_filenames_label[t])
        ...
        fut_filenames_label = self.filenames_label[seq][from_idx: to_idx + 1]
        ...
        for t in range(self.n_future_steps):
            ...
            fut_data[5, t, :, :] = self.load_label(fut_filenames_label[t])
    ...
    def load_label(self, filename):
        """Load .npy label values as (1,height,width) tensor"""
        label = torch.Tensor(np.load(filename)).int()
        return label
...
if __name__ == "__main__":
    ...
    dataset_path = ""
    rawdata_path = ""
    data = KittiOdometryModule(cfg, dataset_path, rawdata_path)
    data.prepare_data()
    data.setup()

    item = data.valid_loader.dataset.__getitem__(1)
    ...
    fig, axs = plt.subplots(6, 1, sharex=True, figsize=(30, 30 * 6 * 64 / 2048))
    ...
    axs[5].imshow(normalize(item["fut_data"][5, 0, :, :].numpy()))
    axs[5].set_title("Label")
