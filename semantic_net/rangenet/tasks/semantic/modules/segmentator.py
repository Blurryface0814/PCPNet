...
from semantic_net.rangenet.tasks.semantic.postproc.CRF import CRF
import importlib
...
class Segmentator(nn.Module):
  def __init__(self, ARCH, nclasses, path=None, path_append="", strict=False):
    ...
    backbone_path = "semantic_net.rangenet.backbones." + self.ARCH["backbone"]["name"]
    bboneModule = importlib.import_module(backbone_path)
    ...
    decoder_path = "semantic_net.rangenet.tasks.semantic.decoders." + self.ARCH["decoder"]["name"]
    decoderModule = importlib.import_module(decoder_path)
    ...
    ### Do not Train any parameter in RangeNet++ (by Zhen Luo)
    # train backbone?
